...
export class SubmainpanelComponent implements AfterViewInit {
  ...
  res: any;
  tradepairs: any[] = [];
  filteredPairs: any[] = []; // Filtered list of trade pairs for display
  ...
  handleGetPairs(data: any) {
    ...
    if (this.coinList.length > 0) {
      this.tradepairs = this.res[selectedTab][this.coinList[0]];
      this.filteredPairs = [...this.tradepairs]; // Initialize filteredPairs with all data
    } else {
      this.tradepairs = [];
      this.filteredPairs = [];
    }
  ...
  //       this.tradepairs = [...this.tradepairs];
  //     },
  //     (error: any) => this.handleError(error)
  //   );
  // }
  ...
  // filterTradepairs() {
  //   // Filter the trade pairs based on the search term
  //   this.tradepairs = this.tradepairs.filter(
  //     (item) =>
  ...
  filterTradepairs() {
    // Reset filteredPairs when searchTerm is empty
    if (!this.searchTerm.trim()) {
      this.filteredPairs = [...this.tradepairs];
    } else {
      // Filter based on searchTerm
      this.filteredPairs = this.tradepairs.filter(
        (item) =>
          item.symbol.toLowerCase().includes(this.searchTerm.toLowerCase()) ||
          item.coinone.toLowerCase().includes(this.searchTerm.toLowerCase())
      );
